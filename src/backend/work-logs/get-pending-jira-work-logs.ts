...
export async function getPendingJiraWorkLogs(): Promise<WorkLogsFetchResult> {
  try {
    ...
    if (error) {
      return {
        status: "error",
        logs: null,
        error: "No se pudieron obtener los registros pendientes de Jira de la base de datos.",
      };
    }
    ...
    return {
      status: "error",
      logs: null,
      error: "Ocurrió un error inesperado al consultar los registros pendientes de Jira.",
    };
  }
}
